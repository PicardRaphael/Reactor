...
const generateRoom = store => next => (action) => {
  switch (action.type) {
    // ----------------------- ROOM CHANGE -----------------------
    case CHANGE_ROOM: {
      console.log('CHANGE_ROOM_MIDDLEWARE');
      ...
      console.log('AVANTenter', enteredDoor);

      // return the current door object providing support for nextRoom reference
      if (enteredDoor !== null) {
        // currentRoom.previousDoor = enteredDoor.id;
        console.log('enteredDoor ELSE', enteredDoor);
    ...
    case GENERATE_LAYERS_ROOM_START: {
      ...
      console.log(callLayer);

      const newcallLayer = callLayer.join('');
      console.log(newcallLayer);
